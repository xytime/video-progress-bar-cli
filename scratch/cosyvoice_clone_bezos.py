"""
Alibaba Cloud CosyVoice Voice Cloning & Subtitle Synthesis for Jeff Bezos 1min Video

Modification History:
- 2026-05-30: Gemini_3.5_Flash_planning: Created Bezos 1min cloning pipeline using Aliyun OSS signed URLs.
"""
...
MODEL_NAME = "cosyvoice-v3-plus"  # High-quality model
SOURCE_VIDEO = "output/Bezos_AI_1min.mp4"
ASS_FILE = "output/Bezos_AI_1min.ass"
REF_AUDIO = "scratch/bezos_ref.wav"
TEMP_DIR = "scratch/temp_bezos"
FINAL_AUDIO_PURE = "output/Bezos_AI_1min_cosyvoice_plus_zh_pure.mp3"
FINAL_VIDEO_PURE = "output/Bezos_AI_1min_cosyvoice_plus_zh_pure.mp4"
FINAL_VIDEO_MIXED = "output/Bezos_AI_1min_cosyvoice_plus_zh_mixed.mp4"
...
def get_or_create_voice_id():
    if OSS_KEY_ID and OSS_KEY_SECRET and OSS_BUCKET:
        ...
        try:
            ...
            # Automatically delete the temporary file from OSS bucket
            try:
                print("  Deleting temporary reference audio from OSS bucket...")
                bucket.delete_object("bezos_ref.wav")
        ...
        print("\n[Error] OSS credentials not fully set in .env.")
        return None

def run_cosyvoice_bezos():
    print("=== Jeff Bezos 1min CosyVoice Voice Cloning & Synthesis ===")
    
    # 1. Get or Enroll Voice ID
    voice_id = get_or_create_voice_id()
    if not voice_id:
        print("Failed to get Voice ID.")
        return
    ...
    # 3. Synthesize each Mandarin segment
    ...
    # 4. Stitch and mix segments using FFmpeg
    ...
    # 5. Assemble output videos
    print("\n[4/4] Assembling final dubbed video outputs...")
    
    # Version A: Pure cloned voice track
    ...
    # Version B: Mixed track with ducked original audio
    ...
    print("\n=== Jeff Bezos 1min CosyVoice Dubbing Pipeline Finished ===")
