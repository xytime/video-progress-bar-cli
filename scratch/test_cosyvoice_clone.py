...
def test_clone():
    print("=== Testing CosyVoice Voice Enrollment ===")
    ...
    # 2. Upload file to DashScope temporary OSS storage
    print("Uploading reference audio to DashScope temporary OSS...")
    try:
        from dashscope.utils.oss_utils import upload_file
        oss_url = upload_file(model="cosyvoice-v3-flash", upload_path=ref_audio, api_key=dashscope.api_key)
