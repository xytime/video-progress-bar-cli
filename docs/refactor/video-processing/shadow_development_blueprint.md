...
| 版本 | 日期 | 作者 | 说明 |
| :--- | :---: | :---: | :--- |
| 2.4.0 | 2026-09-20 | Antigravity | M6.2+ 第七轮架构复核 1 项 P2 闭环：修正 Section 6.1 探测代码块内容多出 1 个空格的缩进缺陷，与围栏缩进严格归零对齐，杜绝从 Markdown 提取后 Python 报 IndentationError 语法错误；在 POLARIS-101 明确验收测试必须动态提取蓝图命令并执行完整语法与逻辑验证 |
...
- **执行**：
  ...
  2. **全面扩展真实入口级测试边界**：
     ...
     - **外键兼容与领取失败原子回滚无残留测试 (`test_claim_attempt_rollback_leaves_no_residual_on_failure`)**：断言在 `BEGIN IMMEDIATE` 事务中，若步骤 3（插入租约表）遭遇主键冲突或外部异常失败，整个事务干净回滚，步骤 2 先行写入的 Attempt 审计行**必须完全回滚，数据库无任何残留孤儿数据**；
      - **微信真实会话锁规范化探测与持锁反例测试 (`test_wechat_session_lock_probe_detects_real_hold_and_release`)**：测试必须**动态提取 `shadow_development_blueprint.md:Section 6.1` 第五小步中的完整命令并执行语法与逻辑验收**（对齐调度器 `pipeline_manager.py:996` 与上传入口 `wechat_uploader.py:1230` 默认约定的 `output/wechat_state.json` 调用 `canonical_wechat_session_lock_path` 派生出的真实锁文件 `.wechat_state.json.browser.lock`）；断言当后台线程通过 `WeChatSessionLock` 持有该真实锁时，完整探测命令**必须捕获 `BlockingIOError` 并判定为锁忙失败（退出码 1 反例，拒绝误报 FREE）**；持锁线程释放后，完整探测命令必须成功断言为 FREE（返回码 0），彻底杜绝缩进语法错误、配置脱节或仅测试底层路径函数带来的隐患；
- **验收护栏 (Guardrails)**：
  - [ ] **严禁修改任何生产代码**。
