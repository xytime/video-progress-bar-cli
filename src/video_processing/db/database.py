...
class PipelineDB:
    ...
    def __init__(self, db_path: str = "pipeline.db", *, read_only: bool = False):
        ...
        if self._read_only:
            ...
            self._init_db()
            self.reconcile_pending_ghost_tasks()
                     ...
                     AND (
                         pv.slice_index = 0 OR NOT EXISTS (
                             ...
                             WHERE sibling.parent_id = pv.parent_id
                               ...
                               AND sibling.status NOT IN ('PUBLISHED', 'IGNORED', 'COMPLETED', 'HISTORICAL_ARCHIVED')
                         )
                     )
    ...
    def promote_to_manual(self, youtube_id: str, score: int = 100) -> bool:
        ...
        并打上手动评分锁（is_manually_scored=1），清除旧错误信息，使其脱离「仅浏览」防火墙、
        正常进入处理/发布队列。保留已抓取的元数据与 zh_title，不经过黑名单墓碑。

        仅当 source='DISCOVERY' 且未在微信平台账本或归档中时生效，避免误改已在正式队列或已发布的任务。

        Returns:
            True 表示成功转换（命中一行）；False 表示视频不存在、已在微信发布/归档中或来源不是 DISCOVERY。
        """
        with self.get_connection() as conn:
            cursor = conn.execute(
                """UPDATE processed_videos SET source = 'MANUAL', score = ?, 
                   is_manually_scored = 1, error_msg = NULL, updated_at = CURRENT_TIMESTAMP 
    ...
    def promote_waitlist_to_queue(self, youtube_id: str, score: int = 100) -> bool:
        """将待筛选(waitlist)或已过期/忽略的素材提权为手动加急任务(MANUAL)，赋分>=75并入队。

        原子地将主任务(slice_index=0) source 提升为 MANUAL、score 设为指定分数(默认100)，
        打上手动评分锁(is_manually_scored=1)，清除历史错误信息，状态重置为 PENDING，移入排队队列。
        受微信账本与历史归档保护，已在微信发布的视频拒绝提权。
        """
        with self.get_connection() as conn:
            cursor = conn.execute(
                """UPDATE processed_videos
                   SET source = 'MANUAL',
                       score = ?,
                       is_manually_scored = 1,
                       status = 'PENDING',
                       error_msg = NULL,
                       updated_at = CURRENT_TIMESTAMP
                   WHERE youtube_id = ?
                     AND slice_index = 0
                     AND status IN ('PENDING', 'EXPIRED', 'IGNORED')
    ...
    def ignore_video(self, youtube_id: str, slice_index: Optional[int] = None, reason: str = "人工忽略") -> bool:
        """将指定任务标记为已忽略(IGNORED)，使其从待处理/待筛选队列退出。
        若指定 slice_index，则仅忽略该切片；若为 None，则忽略该 youtube_id 下的主任务及所有切片。
        受微信保护状态(PUBLISHED, HISTORICAL_ARCHIVED, 审核中等)及微信账本保护，已发布/审核任务禁止覆盖为 IGNORED。
        """
        with self.get_connection() as conn:
            if slice_index is not None:
                cursor = conn.execute(
                    """UPDATE processed_videos
                       SET status = 'IGNORED',
                           ...
                           updated_at = CURRENT_TIMESTAMP
                       WHERE youtube_id = ? AND slice_index = ?
                         AND status NOT IN ('PUBLISHED', 'HISTORICAL_ARCHIVED', 'UNDER_REVIEW', 'SUBMITTED_BOUND', 'SUBMITTED_UNBOUND', 'UNCERTAIN')
                         AND NOT EXISTS (SELECT 1 FROM wechat_publications wp WHERE wp.video_id = processed_videos.id)
                         AND NOT EXISTS (SELECT 1 FROM wechat_publications_historical_archive archive WHERE archive.video_id = processed_videos.id)""",
                    (reason, youtube_id, int(slice_index))
                )
            else:
                cursor = conn.execute(
                    """UPDATE processed_videos
    ...
    def discard_stale_queue_videos(self, stale_days: Optional[int] = None) -> int:
        """批量放弃/忽略排队过久的高分过时视频 (Stale Discard)。

        将滞留在 PENDING 状态超过 stale_days 天的高分任务(score >= 75)
        流转为 IGNORED 状态，不再占用待处理排队队列。排除已在微信发布或归档中的幽灵任务。

        Args:
            stale_days: 排队超期天数，默认读取 settings.queue_stale_days (默认 7 天)

        Returns:
            流转为 IGNORED 的记录数
        """
        if stale_days is None:
            stale_days = settings.queue_stale_days
        days = max(1, int(stale_days))
        with self.get_connection() as conn:
            cursor = conn.execute(
                """UPDATE processed_videos
                   SET status = 'IGNORED',
                       error_msg = COALESCE(error_msg, '超期排队批量放弃'),
                       updated_at = CURRENT_TIMESTAMP
            ...
            if count > 0:
                conn.commit()
                self._logger.info(f"[Queue] Discarded {count} stale queue videos older than {days} days.")
            return count

    def cleanup_expired_waitlist_videos(self, ttl_days: Optional[int] = None) -> int:
        """待筛选(waitlist)低分素材(score < 75) TTL 时效淘汰与归档。

        将抓取时间或原始发布时间超过 ttl_days 天、尚未进入排队的低分素材流转为 EXPIRED 归档状态，
        移出待筛选列表，避免陈年低分素材堆积淹没待筛选首页。保留原始记录与数据血缘。

        Args:
            ttl_days: 淘汰时效天数，默认读取 settings.waitlist_ttl_days (默认 30 天)

        Returns:
            流转为 EXPIRED 的记录数
        """
        if ttl_days is None:
            ttl_days = settings.waitlist_ttl_days
        ...
        with self.get_connection() as conn:
                ...
                """UPDATE processed_videos
                   ...
                   WHERE status = 'PENDING'
                     ...
                     AND (
                         ...
                         OR (upload_date IS NOT NULL AND upload_date != '' AND upload_date GLOB '[12][0-9][0-9][0-9]*' AND SUBSTR(REPLACE(upload_date, '-', ''), 1, 8) < ?)
                     )
                     AND NOT EXISTS (SELECT 1 FROM wechat_publications wp WHERE wp.video_id = processed_videos.id)
                     AND NOT EXISTS (SELECT 1 FROM wechat_publications_historical_archive archive WHERE archive.video_id = processed_videos.id)""",
            ...
            if count > 0:
                conn.commit()
                self._logger.info(f"[TTL] Evicted {count} expired waitlist videos older than {days} days.")
            return count

    def reconcile_pending_ghost_tasks(self) -> int:
        """自愈校准幽灵待处理(PENDING)任务状态。

        将已存在于微信历史归档表(wechat_publications_historical_archive)中的
        PENDING 记录校准为 HISTORICAL_ARCHIVED；
        将已存在于微信发布账本(wechat_publications)中的 PENDING 记录按其真实
        平台终态(PUBLISHED / REJECTED / UNDER_REVIEW 等)对齐。
        避免已被底层调度器硬过滤的历史残留继续误报为活跃待处理任务。
        ...
        calibrated = 0
        with self.get_connection() as conn:
            # 1. 历史归档表中的 PENDING 条目 -> 校准为 HISTORICAL_ARCHIVED
            cursor = conn.execute(
                """UPDATE processed_videos
                   SET status = 'HISTORICAL_ARCHIVED',
                       updated_at = CURRENT_TIMESTAMP
                   WHERE status = 'PENDING'
                     AND EXISTS (
                         SELECT 1 FROM wechat_publications_historical_archive archive
                         WHERE archive.video_id = processed_videos.id
                     )"""
            )
            calibrated += cursor.rowcount

            # 2. 微信发布账本中的 PENDING 条目 -> 校准为对应的实际状态
            cursor = conn.execute(
                """SELECT pv.id, wp.state, wp.last_error_message
                   FROM processed_videos pv
                   JOIN wechat_publications wp ON wp.video_id = pv.id
                   WHERE pv.status = 'PENDING'"""
            )
            ...
            for r in pub_rows:
                ...
                if state == "PUBLISHED":
                    conn.execute(
                        "UPDATE processed_videos SET status = 'PUBLISHED', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                        (v_id,)
                    )
                elif state == "REJECTED":
                    conn.execute(
                        "UPDATE processed_videos SET status = 'FAILED', error_msg = COALESCE(?, error_msg, '微信平台审核驳回'), updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                        (err, v_id)
                    )
                elif state in ("UNDER_REVIEW", "SUBMITTED_BOUND", "SUBMITTED_UNBOUND", "UNCERTAIN"):
                    conn.execute(
                        "UPDATE processed_videos SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                        (state, v_id)
                    )
                else:
                    conn.execute(
                        "UPDATE processed_videos SET status = 'HISTORICAL_ARCHIVED', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                        (v_id,)
                    )
                calibrated += 1
            ...
            if calibrated > 0:
                conn.commit()
                self._logger.info(f"[SelfHealing] Reconciled {calibrated} ghost pending records.")
        return calibrated
    ...
    def _build_video_tab_condition(
        self, tab: str, engagement_window_days: int, include_expired: bool = False
    ) -> tuple[str, List[Any]]:
        """返回列表 Tab 的基础谓词；调用方再安全叠加筛选且始终在分页前执行。"""
        if tab == 'completed':
            return """((pv.status IN ('PUBLISHED', 'IGNORED', 'COMPLETED', 'HISTORICAL_ARCHIVED') AND pv.parent_id IS NULL)
                OR (pv.status = 'SEGMENTED' AND pv.parent_id IS NULL AND
                    (SELECT COUNT(*) FROM processed_videos sub WHERE sub.parent_id = pv.id
                     AND sub.status NOT IN ('PUBLISHED', 'IGNORED', 'COMPLETED', 'HISTORICAL_ARCHIVED')) = 0))""", []
        if tab == 'error':
    ...
    def get_paginated_videos(
        ...
        *,
        ...
        if funnel_stage:
            ...
            condition, params = self._build_video_tab_condition(
                tab, engagement_window_days, include_expired=include_expired
            )
            clauses = [condition]
        ...
        with self.get_connection() as conn:
            ...
            rows = conn.execute(
                f"""SELECT pv.*, COALESCE(rc.channel_name, pv.channel_id) AS channel_name,
                           ...
                           (SELECT COUNT(*) FROM processed_videos sub WHERE sub.parent_id = pv.id AND sub.status IN ('PUBLISHED', 'IGNORED', 'COMPLETED', 'HISTORICAL_ARCHIVED')) AS completed_slices_count
                    FROM processed_videos pv
                    LEFT JOIN recommended_channels rc ON pv.channel_id = rc.channel_id
                    WHERE {where}
        ...
        with self.get_connection() as conn:
            cursor = conn.execute("""
                SELECT
                    ...
                    SUM(CASE WHEN (
                        pv.status IN ('PUBLISHED', 'IGNORED', 'COMPLETED', 'HISTORICAL_ARCHIVED')
                        OR
                        (pv.status = 'SEGMENTED' AND 
                         (SELECT COUNT(*) FROM processed_videos sub WHERE sub.parent_id = pv.id AND sub.status NOT IN ('PUBLISHED', 'IGNORED', 'COMPLETED', 'HISTORICAL_ARCHIVED')) = 0)
    ...
    def authorize_english_world_douyin_cover_repair_recovery(
        self, review_id: str, *, reason: str,
    ) -> Dict[str, Any]:
        """为第三次双封面检测明确拒绝、且从未提交的记录签发最后一次人工恢复。

        此入口只接受一个极窄的三次证据链：第一轮是旧 ``exit=7`` 但日志明确
        “未保存草稿、未发布”，后两轮均是当前上传器的 ``exit=3`` 发布前停止，
        最后一轮还必须记录平台 ``横/竖双封面缺失``。它只在封面保存流程修复后
        由人工调用一次；任何未知提交、第四次失败或不同页面错误均 fail-closed。
        """
        clean_review_id = (review_id or "").strip()
        clean_reason = " ".join((reason or "").split())[:500]
        if not clean_review_id:
        ...
        with self.get_connection() as conn:
            ...
            publication = conn.execute(
                """SELECT review_id FROM english_world_douyin_publications
                   WHERE review_id = ? AND state = 'CANCELED' AND attempt_count = 3
                     AND submitted_at IS NULL AND recovery_authorized_at IS NULL
            ...
            if not publication or not attempts or not (
                int(attempts["total"] or 0) == 3
                and int(attempts["canceled"] or 0) == 3
                and int(attempts["legacy_exit_seven"] or 0) == 1
                and int(attempts["pre_submit_exit_three"] or 0) == 2
                and int(attempts["explicitly_not_submitted"] or 0) >= 1
                and int(attempts["dual_cover_missing"] or 0) >= 1
            ):
                conn.rollback()
                raise ValueError(
                    "Only three proven pre-submit English World Douyin stops can receive cover repair recovery",
                )
            cursor = conn.execute(
                """UPDATE english_world_douyin_publications
                   SET state = 'QUEUED', recovery_authorized_at = CURRENT_TIMESTAMP,
            ...
            if cursor.rowcount != 1:
                conn.rollback()
                raise ValueError("English World Douyin cover repair recovery changed concurrently")
            conn.commit()
            return self.get_english_world_douyin_publication(clean_review_id) or {}

    def authorize_english_world_douyin_persisted_cover_recovery(
        self, review_id: str, *, reason: str,
    ) -> Dict[str, Any]:
        """为四次双封面发布前停止签发一次“卡槽已落库”修复验证。

        这不是通用重试开关：仅允许首轮旧 ``exit=7`` 明确未提交、其后三轮
        ``exit=3`` 且均为双封面缺失的同一审核项。第五次领取只用于验证最新的
        卡槽缩略图落库闸门；一旦再失败、已有提交或账本证据不全，永久停止。
        """
        clean_review_id = (review_id or "").strip()
        clean_reason = " ".join((reason or "").split())[:500]
        if not clean_review_id:
        ...
        with self.get_connection() as conn:
            ...
            if not publication or not attempts or not (
                int(attempts["total"] or 0) == 4
                and int(attempts["canceled"] or 0) == 4
                and int(attempts["legacy_exit_seven"] or 0) == 1
                and int(attempts["pre_submit_exit_three"] or 0) == 3
                and int(attempts["explicitly_not_submitted"] or 0) >= 1
                and int(attempts["dual_cover_missing"] or 0) >= 2
            ):
                conn.rollback()
                raise ValueError(
                    "Only four proven pre-submit English World Douyin stops can receive persisted cover recovery",
                )
            cursor = conn.execute(
                """UPDATE english_world_douyin_publications
                   SET state = 'QUEUED', recovery_authorized_at = CURRENT_TIMESTAMP,
