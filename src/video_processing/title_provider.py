    ...
    *,
    ...
    try:
        payload = run_agy_structured(
            _build_agy_title_prompt(title=title, description=description),
            schema=schema,
            model=model,
            command=agy_bin,
    ...
    except (AgyProviderError, KeyError, TypeError, ValueError, TitleContractError) as exc:
        raise TitleProviderError(f"agy 标题输出无效：{type(exc).__name__}") from exc


def _build_agy_title_prompt(*, title: str, description: str) -> str:
