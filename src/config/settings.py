...
| 3.54.0 | 2026-08-29 | Codex | Dashboard 绑定地址收口为回环白名单，禁止配置回退到 0.0.0.0 暴露生产写接口。 |
...
class Settings(BaseSettings):
    ...
    # 控制面默认监听全部 IPv4 网卡，供同一局域网设备使用；浏览器来源校验在
    # web.app 中继续生效。仅允许明确的回环地址或 IPv4 通配地址，拒绝任意主机名。
    dashboard_bind_host: str = Field(
        default="0.0.0.0",
        pattern=r"^(127\.0\.0\.1|::1|0\.0\.0\.0)$",
    )
