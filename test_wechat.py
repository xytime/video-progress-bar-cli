...
def run_uploader(
    ...
    with sync_playwright() as p:
        ...
        try:
            print("Hovering over cover...")
            page.locator("text=封面预览").locator("xpath=..").hover()
            page.wait_for_timeout(2000)
            print("Clicking exact text '编辑'...")
            page.locator("text=封面预览").locator("xpath=..").locator("text='编辑'").first.click(force=True)
            page.wait_for_timeout(2000)
            print("Dumping modal buttons...")
            btns = page.evaluate('''() => {
